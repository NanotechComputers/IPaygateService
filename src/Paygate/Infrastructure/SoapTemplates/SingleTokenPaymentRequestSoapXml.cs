...
{
    internal static class SingleTokenPaymentRequestSoapXml
    {
        private static string Get(string merchantId, string merchantSecret, CreateTokenTransactionModel data, string userDefinedFieldXml)
        {
            ...
            {
                throw new NullReferenceException(nameof(data.Token));
            }

            if (string.IsNullOrEmpty(data.TokenDetail))
            {
                throw new NullReferenceException(nameof(data.TokenDetail));
            }
            ...
            if (data.Vault)
            {
                vaultXml = $"<pay:Vault>True</pay:Vault>";
            }

            var billingDescriptorXml = "";
            if (!string.IsNullOrWhiteSpace(data.BillingDescriptor))
            {
                billingDescriptorXml = $"<pay:BillingDescriptor>{data.BillingDescriptor}</pay:BillingDescriptor>";
            }
                        ...
                        <soapenv:Body>
                            <pay:SinglePaymentRequest>
                                <pay:TokenPaymentRequest>
                                    ...
                                    <pay:Token>{data.Token ?? ""}</pay:Token>
                                    <pay:TokenDetail>{data.TokenDetail ?? ""}</pay:TokenDetail>
                                    {vaultXml}
                                    {billingDescriptorXml}
                                    <pay:Order>
        ...
        internal static string Get(string merchantId, string merchantSecret, CreateTokenTransactionModel data)
        {
            return Get(merchantId, merchantSecret, data, "");
        }

        internal static string Get<TUserdefined>(string merchantId, string merchantSecret, CreateTokenTransactionModel<TUserdefined> data) where TUserdefined : class
        {
